...
class SolarNet(nn.Module):
    def __init__(self, in_channels=3, n_class=6):
        ...
        # Upsample blocks
        self.upscore2e = nn.Upsample(scale_factor=2, mode='bilinear', align_corners=True)
        self.upscore3e = nn.Upsample(scale_factor=4, mode='bilinear', align_corners=True)
        self.upscore4e = nn.Upsample(scale_factor=8, mode='bilinear', align_corners=True)
        ...
        self.outconv1 = nn.Conv2d(64, 6, 3, padding=1)  # 6 roof orientation classes
    ...
    def forward(self, x):
        ...
        hx = torch.cat((h1e, h2e, h3e, h4e), 1)
        hx = self.edgev(hx)
        fedge = self.edgef(hx)
